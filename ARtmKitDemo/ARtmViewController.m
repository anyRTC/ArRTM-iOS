...
- (void)viewDidLoad {
    ...
    if (self.rtmType) {
        self.textLabel.text = [NSString stringWithFormat:@"ChannelId：%@",self.acount];
        self.rtmChannel = [ARtmManager.rtmKit createChannelWithId:self.acount delegate:self];
        [self.rtmChannel joinWithCompletion:^(ARtmJoinChannelErrorCode errorCode) {
            
        }];
    } else {
        self.textLabel.text = [NSString stringWithFormat:@"Peer：%@",self.acount];
    }
...
- (IBAction)doSomethingEvent:(UIButton *)sender {
    switch (sender.tag) {
        case 50:
            if (self.rtmType) {
                [ARtmManager.rtmKit destroyChannelWithId:self.acount];
            }
...
- (void)sendRtmMessage {
    ...
    if (self.rtmType) {
        ...
        [ARtmManager.rtmKit sendMessage:message toPeer:self.acount sendMessageOptions:options completion:^(ARtmSendPeerMessageErrorCode errorCode) {
...
- (void)sendMessageResult:(NSString *)text {
    ARtmMessageModel *model = [[ARtmMessageModel alloc] init];
    model.content = text;
    model.userName = ARtmManager.getLocalUid;
    model.direction = 1;
    [self.dataArr addObject:model];
    [self.tableView reloadData];
    [self scrollToEnd];
