...
文件或图片下载相关错误码。
 */
typedef NS_ENUM(NSInteger, ARtmDownloadMediaErrorCode) {
    
    /**
     0: 方法调用成功，或下载操作成功。
     */
    ARtmDownloadMediaErrorOk = 0,
    
    /**
     1: 通用未知错误。
     */
    ARtmDownloadMediaErrorFailure = 1,
    
    /**
     2: 输入参数错误。比如 mediaId 格式错误或 fileLocaltion 为空。
     */
    ARtmDownloadMediaErrorInvalidArgument = 2,
    
    /**
     3: 服务器响应超时。当前如果 SDK 与文件服务器之间没有文件传输达 120 秒时，SDK 认定下载超时。
     */
    ARtmDownloadMediaErrorTimeout = 3,
    /**
     4: 下载文件或图片不存在，可能因为 mediaId 输入错误，也可能因为 mediaId 已过期。
     */
    ARtmDownloadMediaErrorNotExist = 4,
    /**
     5: 文件或图片同时下载数超过 9 个的上限。
     */
    ARtmDownloadMediaErrorConcurrencyLimitExceeded = 5,
    /**
     6: 文件或图片下载任务因如下原因中途退出:
     * 下载方处于 ARtmConnectionStateAborted 状态。
     * 下载方取消了文件或图片下载。
     */
    ARtmDownloadMediaErrorInterrupted = 6,
    
    /**
     101: SDK 未完成初始化。
     */
    ARtmDownloadMediaErrorNotInitialized = 101,
    
    /**
     102: 本次操作前未调用 loginByToken 方法或者 loginByToken 方法调用未成功。
     */
    ARtmDownloadMediaErrorNotLoggedIn = 102,
};

/**
文件或图片上传相关错误码。
 */
typedef NS_ENUM(NSInteger, ARtmUploadMediaErrorCode) {
    
    /**
     0: 方法调用成功，或上传操作成功。
     */
    ARtmUploadMediaErrorOk = 0,
    
    /**
     1: 通用未知错误。
     */
    ARtmUploadMediaErrorFailure = 1,
    
    /**
     2: 输入参数错误。比如 mediaId 格式错误。
     */
    ARtmUploadMediaErrorInvalidArgument = 2,
    
    /**
     3: 服务器响应超时。当前如果 SDK 与文件服务器之间没有文件传输达 120 秒时，SDK 认定上传超时。
     */
    ARtmUploadMediaErrorTimeout = 3,
    /**
     4: 上传文件或图片大小超过 30 MB 上限。
     */
    ARtmUploadMediaErrorSizeOverflow = 4,
    /**
     5: 文件或图片同时上传数超过 9 个的上限。
     */
    ARtmUploadMediaErrorConcurrencyLimitExceeded = 5,
    /**
     6: 文件或图片上传任务因如下原因中途退出:
     * 上传方处于 ARtmConnectionStateAborted 状态。
     * 上传方取消了文件或图片上传。
     */
    ARtmUploadMediaErrorInterrupted = 6,
    
    /**
     101: SDK 未完成初始化。
     */
    ARtmUploadMediaErrorNotInitialized = 101,
    
    /**
     102: 本次操作前未调用 loginByToken 方法或者 loginByToken 方法调用未成功。
     */
    ARtmUploadMediaErrorNotLoggedIn = 102,
};
...
取消上传或取消下载相关的错误码。
 */
typedef NS_ENUM(NSInteger, ARtmCancelMediaErrorCode) {
    
    /**
     0: 方法调用成功，或取消操作成功。
     */
    ARtmCancelMediaErrorOk = 0,
    
    /**
     1: 通用未知错误。
     */
    ARtmCancelMediaErrorFailure = 1,
    
    /**
     2: 待取消任务不存在。你只能取消正在进行中的上传或下载任务。如果上传或下载任务已完成，对应的 requestId 将不再有效。
     */
    ARtmCancelMediaErrorNotExist = 2,
    
    /**
     101: SDK 未完成初始化。
     */
    ARtmCancelMediaErrorNotInitialized = 101,
    
    /**
     102: 本次操作前未调用 loginByToken 方法或者 loginByToken 方法调用未成功。
     */
    ARtmCancelMediaErrorNotLoggedIn = 102,
};
